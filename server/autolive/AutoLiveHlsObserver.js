...
const safeFailure=e=>e?.code==='HTTP_ERROR'?{httpStatus:Number.isInteger(e.details?.status)?e.details.status:null,httpStage:e.details?.stage||null}:null;
    ...
    async sample(source,signal){
        try{
            ...
            for(let depth=0;depth<3;depth++){
                const stage=depth===0?'manifest':'variant';
                const response=await this.http.read(url,{signal,stage});url=response.url;playlist=parsePlaylist(response.body,url);
            ...
            const segment=await this.http.read(last.uri,{signal,segment:true,stage:'segment'});if(!segment.bytes)throw error('SEGMENT_EMPTY');
            ...
            const diagnostics=safeFailure(e);if(diagnostics)e.diagnostics=diagnostics;throw e;
        }
    }
}

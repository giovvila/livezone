...
export default class AutoLiveRoutes {
    ...
    async handle(req,res,url){
        ...
        const stageObservation=url.pathname==='/api/studio/autolive/browser-stage';
        if(!migration&&!stageObservation&&url.pathname!=='/api/studio/autolive')return send(res,404,{ok:false,error:{code:'NOT_FOUND'}});
        if(req.method==='GET'&&!migration&&!stageObservation)return send(res,owner.available()?200:503,{ok:owner.available(),...owner.current()},
        ...
        if(stageObservation){
            ...
            try{
                ...
                for await(const chunk of req.iterator({destroyOnReturn:false})){size+=chunk.length;if(size>1024){req.resume();const e=new Error();e.code='PAYLOAD_TOO_LARGE';throw e;}chunks.push(chunk);}
                let value;try{value=JSON.parse(Buffer.concat(chunks).toString('utf8'));}catch{const e=new Error();e.code='INVALID_JSON';throw e;}
                const keys=Object.keys(value||{});if(!value||typeof value!=='object'||Array.isArray(value)||keys.length!==1||keys[0]!=='stage'||!['LIVE','INACTIVE'].includes(value.stage)){
                    const e=new Error();e.code='INVALID_STAGE';throw e;
                }
                owner.observeBrowserStage(value.stage==='LIVE'?'LIVE':null);
                return send(res,200,{ok:true,runtime:owner.current().runtime});
            }catch(e){const code=e.code||'INVALID_REQUEST';const status=code==='PAYLOAD_TOO_LARGE'?413:code==='INVALID_JSON'?400:422;

...
test('10-second loss recovers same LIVE session without A or ENTRY',async()=>harness(async h=>{
 await h.detect();await h.advance(61000);const session=h.controller.session;
 h.monitor.emit('OFFLINE');h.renderer.program.renderer.video.dispatchEvent(new Event('waiting'));await h.time.advance(10000);
 assert.equal(h.controller.session,session);h.monitor.emit('ONLINE');h.progress();await flush();
 assert.deepEqual(history(h.published),['A','ENTRY','LIVE','LOSS','LIVE']);assert.equal(h.scheduler.ends,0);
}));

for(const duration of [1000,3000,5000,10000,14999])test(duration+'ms interruption recovers same session and context',async()=>harness(async h=>{
 await h.detect();await h.advance(61000);const session=h.controller.session,context=h.scheduler.context,owner=h.controller.activeHealth;
 h.monitor.emit('OFFLINE');h.renderer.program.renderer.video.dispatchEvent(new Event('waiting'));
 await h.time.advance(duration);assert.equal(h.controller.session,session);assert.equal(h.scheduler.ends,0);
 h.monitor.emit('ONLINE');h.progress();await flush();assert.equal(h.controller.session,session);assert.equal(h.scheduler.context,context);assert.equal(h.controller.activeHealth,owner);
 assert.equal(h.controller.lossTimer,null);assert.equal(h.controller.lossGraceDeadline,null);assert.equal(owner.confirmedSince,null);
 ...
 await h.advance(20000);assert.equal(h.scheduler.begins,1);assert.equal(h.scheduler.ends,0);
}));
test('exact continuous 15-second absence restores cue once, then genuine return requires a new full gate',async()=>harness(async h=>{
 const wasEnabled=trace.enabled;trace.enabled=true;trace.clear();try{
 await h.detect();await h.advance(61000);const session=h.controller.session,started=h.time.now();
 h.monitor.emit('OFFLINE');h.renderer.program.renderer.video.dispatchEvent(new Event('waiting'));
 await h.time.advance(2000);assert.equal(h.controller.lossGraceDeadline,started+AUTOLIVE_POST_TAKE_CONFIRMED_LOSS_MS);
 await h.time.advance(AUTOLIVE_POST_TAKE_CONFIRMED_LOSS_MS-2001);assert.equal(h.controller.session,session);assert.equal(h.scheduler.ends,0);
 await h.time.advance(1);await flush();assert.equal(h.controller.session,null);assert.equal(h.scheduler.ends,1);
 assert.deepEqual(history(h.published),['A','ENTRY','LIVE','LOSS','A']);assert.equal(h.renderer.program.renderer.video.currentTime,37);assert.equal(h.state.getPreviewSceneId(),'P');
 await h.time.advance(20000);assert.equal(h.scheduler.ends,1);await h.detect();assert.notEqual(h.controller.session.sessionId,session.sessionId);
 await h.advance(29000);assert.equal(h.controller.session.phase,'PREPARING');await h.advance(2000);assert.equal(h.controller.session.phase,'LIVE');
 assert.deepEqual(history(h.published),['A','ENTRY','LIVE','LOSS','A','ENTRY','LIVE']);
 writeFileSync('var/confirmed-loss-persistent-trace.json',trace.exportJSON());
 }finally{trace.enabled=wasEnabled;trace.clear();}
}));
test('recovery invalidates old timer and a second loss gets a full new deadline',async()=>harness(async h=>{
 await h.detect();await h.advance(61000);const session=h.controller.session;
 h.monitor.emit('OFFLINE');h.renderer.program.renderer.video.dispatchEvent(new Event('waiting'));await h.time.advance(3000);
 const stale=h.time.timers.get(h.controller.lossTimer).fn;h.monitor.emit('ONLINE');h.progress();await flush();
 await h.advance(2000);const start=h.time.now();h.monitor.emit('OFFLINE');h.renderer.program.renderer.video.dispatchEvent(new Event('waiting'));await h.time.advance(2000);
 assert.equal(h.controller.lossGraceDeadline,start+AUTOLIVE_POST_TAKE_CONFIRMED_LOSS_MS);stale();
 await h.time.advance(AUTOLIVE_POST_TAKE_CONFIRMED_LOSS_MS-2001);assert.equal(h.controller.session,session);
 h.monitor.emit('ONLINE');h.progress();await flush();stale();assert.equal(h.scheduler.ends,0);assert.equal(h.controller.session,session);
}));
test('source availability cancels loss deadline while recovering decoder remains covered',async()=>harness(async h=>{
 await h.detect();await h.advance(61000);const session=h.controller.session;
 h.monitor.emit('OFFLINE');h.renderer.program.renderer.video.dispatchEvent(new Event('waiting'));await h.time.advance(10000);
 const stale=h.time.timers.get(h.controller.lossTimer).fn;h.monitor.emit('ONLINE');await h.time.advance(20000);stale();
 assert.equal(h.controller.session,session);assert.equal(h.controller.lossTimer,null);assert.ok(h.binding.presentation);
 const start=h.time.now();h.monitor.emit('OFFLINE');assert.equal(h.controller.lossGraceDeadline,start+AUTOLIVE_POST_TAKE_CONFIRMED_LOSS_MS);
 await h.time.advance(14999);assert.equal(h.controller.session,session);h.monitor.emit('ONLINE');h.progress();await flush();assert.equal(h.scheduler.ends,0);
}));
test('player rebuild in grace recovers LIVE directly without changing interruption context',async()=>harness(async h=>{
 await h.detect();await h.advance(61000);const session=h.controller.session,context=h.scheduler.context;
 h.monitor.emit('OFFLINE');h.renderer.program.renderer.video.dispatchEvent(new Event('waiting'));await h.time.advance(5000);
 await h.renderer.renderSlot(h.renderer.program,'LIVE');h.monitor.emit('ONLINE');await h.advance(2000);
 assert.equal(h.controller.session,session);assert.equal(h.scheduler.context,context);assert.equal(h.scheduler.begins,1);assert.equal(h.scheduler.ends,0);
 assert.deepEqual(history(h.published),['A','ENTRY','LIVE','LOSS','LIVE']);
}));
for(const action of ['operator','disarm'])test(action+' during LOSS invalidates return deadline safely',async()=>harness(async h=>{
 await h.detect();await h.advance(61000);h.monitor.emit('OFFLINE');h.renderer.program.renderer.video.dispatchEvent(new Event('waiting'));await h.time.advance(3000);
 const stale=h.time.timers.get(h.controller.lossTimer).fn;
 if(action==='operator')assert.ok((await h.command.execute({sceneId:'B',origin:'operator'})).ok);else h.config.change({armed:false});
 await flush();stale();await h.time.advance(30000);assert.equal(h.controller.session,null);assert.equal(h.scheduler.ends,1);
 assert.equal(h.state.getProgramSceneId(),action==='operator'?'B':'A');assert.equal(h.scheduler.begins,1);
}));
test('short-loss trace retains ownership and publishes only LOSS then LIVE',async()=>harness(async h=>{
 const wasEnabled=trace.enabled;trace.enabled=true;trace.clear();try{
 await h.detect();await h.advance(61000);const session=h.controller.session;
 h.monitor.emit('OFFLINE');h.renderer.program.renderer.video.dispatchEvent(new Event('waiting'));await h.time.advance(10000);
 h.monitor.emit('ONLINE');h.progress();await flush();assert.equal(h.controller.session,session);
 assert.deepEqual(history(h.published),['A','ENTRY','LIVE','LOSS','LIVE']);
 assert.ok(!trace.snapshot().some(e=>e.event==='session-close'));
 writeFileSync('var/confirmed-loss-recovery-trace.json',trace.exportJSON());
 }finally{trace.enabled=wasEnabled;trace.clear();}
}));

test('manifest or unavailable frame cannot clear LOSS until playable progress resumes',async()=>harness(async h=>{
 await h.detect();await h.advance(61000);const session=h.controller.session,v=h.renderer.program.renderer.video;
 h.monitor.emit('OFFLINE');v.dispatchEvent(new Event('waiting'));await h.time.advance(5000);
 v.readyState=1;h.monitor.emit('ONLINE');h.progress();await flush();assert.ok(h.binding.presentation);assert.equal(h.controller.session,session);
 await h.time.advance(2000);assert.ok(h.binding.presentation);v.readyState=2;h.progress();await flush();
 assert.equal(h.binding.presentation,null);assert.equal(h.controller.session,session);assert.equal(h.scheduler.begins,1);
 assert.deepEqual(history(h.published),['A','ENTRY','LIVE','LOSS','LIVE']);
}));

...
async function harness(t,{withProgram=true}={}){
 ...
 const h={get server(){return server;},clock,get base(){return 'http://127.0.0.1:'+server.server.address().port;},
  ...
  async publish(snapshot){const response=await fetch(h.base+'/api/program-output',{method:'POST',headers:{Authorization:'Bearer crawl-test-publisher','X-Livezone-Program-Manual':'1','Content-Type':'application/json'},body:JSON.stringify(createProgramOutputEnvelope(snapshot))});assert.equal(response.status,202);},
